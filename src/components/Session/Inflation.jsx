...
export default class Inflation extends React.Component {
  ...
  render() {
    ...
    if (account.inflation_destination === undefined) {
      ...
      inflationDestInfo = <div>
        <p>You are currently voting for: <strong>{account.inflation_destination}</strong> (eQuid)</p>
        <div className="Generic__divider"></div>
        <h3 className="Inflation__thankYou">Thank you for voting for eQuid!</h3>
      </div>
    } else {
      let stButton = <button className="s-button Inflation__stellarTermVote" onClick={(e) => {this.handleStellarTermVote(e)}}>Vote for eQuid</button>
